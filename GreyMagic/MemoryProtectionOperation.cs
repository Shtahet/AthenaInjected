...
namespace GreyMagic
{
    ...
    public class MemoryProtectionOperation : IDisposable
    {
        ...
        /// </remarks>
        public MemoryProtectionOperation(SafeMemoryHandle hProcess, uint address, int size, uint flNewProtect)
        {
            _hProcess = hProcess;
            _address = address;
            _size = size;
...
#else
            VirtualProtect((IntPtr) _address, size, flNewProtect, out _oldProtect);
#endif
        }
...
#else
            VirtualProtect((IntPtr) _address, _size, _oldProtect, out trash);
#endif
        }
