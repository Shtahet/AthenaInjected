...
namespace GreyMagic
{
    public unsafe class InProcessMemoryReader : MemoryBase
    {
        ...
        private T InternalRead<T>(uint address) where T : struct
        {
            try
            {
                ...
                switch (MarshalCache<T>.TypeCode)
                {
                    case TypeCode.Object:
                        ...
                        ret = Marshal.PtrToStructure((IntPtr) address, typeof (T));
                        break;
        ...
        /// </returns>
        public override int WriteBytes(uint address, byte[] bytes, bool isRelative = false)
        {
            if (isRelative)
        ...
        /// <param name="isRelative">if set to <c>true</c> [is relative].</param>
        /// <returns></returns>
        public override byte[] ReadBytes(uint address, int count, bool isRelative = false)
        {
            if (isRelative)
                address = GetAbsolute(address);
        ...
        /*public T Read<T>(uint address, bool isRelative = false) where T : struct
        {
            return Read<T>(address, isRelative);
        }*/
        ...
        /// <returns> . </returns>
        public override T Read<T>(uint address, bool isRelative = false)
        {
            if (isRelative)
                address = GetAbsolute(address);
        ...
        public override bool Write<T>(uint address, T value, bool isRelative = false)
        {
            if (isRelative)
                address = GetAbsolute(address);

            Marshal.StructureToPtr(value, (IntPtr) address, false);
            return true;
        }
        ...
        /// <param name="isRelative"> (optional) the relative. </param>
        /// <returns> . </returns>
        public override T[] Read<T>(uint address, int count, bool isRelative = false)
        {
            int size = MarshalCache<T>.Size;
            var ret = new T[count];
            for (int i = 0; i < count; i++)
            {
                ret[i] = Read<T>((uint) (address + (i*size)), isRelative);
            }
            return ret;
        }
        ...
        /// <returns> . </returns>
        public override T Read<T>(bool isRelative = false, params uint[] addresses)
        {
            ...
            var temp = Read<uint>(addresses[0], isRelative);

            for (int i = 1; i < addresses.Length - 1; i++)
            {
                temp = Read<uint>((uint) (temp + (int) addresses[i]));
            }
            return Read<T>((uint) (temp + (int) addresses[addresses.Length - 1]));
        }
        ...
        /// <param name="addresses"> A variable-length parameters list containing addresses. </param>
        /// <returns> true if it succeeds, false if it fails. </returns>
        public override bool Write<T>(bool isRelative = false, T value = default(T), params uint[] addresses)
        {
            if (addresses.Length == 0)
            {
            ...
            return Write((uint) (temp + (int) addresses[addresses.Length - 1]), value);
        }
        #endregion
    }
}